// ============================================================
// AURA Chat Panel — Dev Handoff Generator v2
// Fixed: font loading, simplified structure, robust error handling
// ============================================================
...
const C = {
  white:        {r:1, g:1, b:1},
  chatBody:     {r:0.969, g:0.973, b:0.984},      // #f7f8fb
  assistBubble: {r:0.902, g:0.941, b:0.984},       // #E6F0FB
  userBubble:   {r:0.957, g:0.961, b:0.980},       // #F4F5FA
  successBg:    {r:0.925, g:0.992, b:0.953},        // #ecfdf3
  successBdr:   {r:0.722, g:0.894, b:0.784},        // #b8e4c8
  successTxt:   {r:0.086, g:0.396, b:0.204},        // #166534
  cardBdr:      {r:0.847, g:0.863, b:0.902},        // #d8dce6
  hdrBdr:       {r:0.898, g:0.906, b:0.922},        // #e5e7eb
  compBdr:      {r:0.788, g:0.796, b:0.824},        // #c9cbd2
  primary:      {r:0.039, g:0.408, b:0.859},        // #0a68db
  txtDark:      {r:0.067, g:0.098, b:0.165},        // #111827
  txtBody:      {r:0.2, g:0.2, b:0.2},              // #333333
  txtMuted:     {r:0.361, g:0.361, b:0.361},         // #5c5c5c
  txtPlaceholder:{r:0.533, g:0.533, b:0.533},        // #888888
  txtSlate:     {r:0.392, g:0.455, b:0.545},         // #64748B
  typingDot:    {r:0.435, g:0.478, b:0.553},         // #6f7a8d
  sparkleBg:    {r:0.914, g:0.961, b:1},             // #e9f5ff
  sparkleIcon:  {r:0.031, g:0.408, b:0.859},         // #0868db
  emptyCircle:  {r:0.945, g:0.961, b:0.976},         // #f1f5f9
  gradStart:    {r:0.2, g:0.78, b:0.918},            // #33C7EA
  gradEnd:      {r:0.165, g:0.176, b:0.733},         // #2A2DBB
  recCardBg:    {r:0.941, g:0.945, b:0.965},         // #f0f1f6
  summaryBdr:   {r:0.812, g:0.827, b:0.867},         // #cfd3dd
  tblHdrBdr:    {r:0.902, g:0.914, b:0.941},         // #e6e9f0
  rowSep:       {r:0.933, g:0.941, b:0.957},         // #eef0f4
  annBg:        {r:1, g:0.95, b:0.8},                // warm yellow
  annBdr:       {r:0.9, g:0.82, b:0.5},
  purple:       {r:0.357, g:0.165, b:0.851},         // #5b2ad9
  compTopBdr:   {r:0.886, g:0.898, b:0.925},         // #e2e5ec
};

function solid(c: {r:number,g:number,b:number}, opacity?: number): SolidPaint {
  const p: SolidPaint = {type:"SOLID", color:c};
  if (opacity !== undefined) p.opacity = opacity;
  return p;
}

// Font state
let FF = "Inter"; // will be set after loading
let HAS_MEDIUM = false;
let HAS_BOLD = false;

function fontName(weight: "Regular"|"Medium"|"Bold"): FontName {
...
async function loadFonts() {
  // Try Inter first (most reliable in Figma), then Roboto
  ...
  try {
    ...
    HAS_MEDIUM = true;
  } catch(e) { console.log("No Medium for " + FF); }
  // Try Bold
  try {
    await figma.loadFontAsync({family:FF, style:"Bold"});
    HAS_BOLD = true;
  } catch(e) { console.log("No Bold for " + FF); }
  console.log("Font ready: " + FF + " Medium=" + HAS_MEDIUM + " Bold=" + HAS_BOLD);
}

// --- Primitives ---

function txt(content: string, size: number, color: {r:number,g:number,b:number}, weight:"Regular"|"Medium"|"Bold"="Regular", width?: number): TextNode {
...
function rect(w: number, h: number, fill: {r:number,g:number,b:number}): RectangleNode {
...
function autoFrame(name: string, dir: "HORIZONTAL"|"VERTICAL", gap: number, pt: number, pr: number, pb: number, pl: number): FrameNode {
...
function fixedAutoFrame(name: string, dir: "HORIZONTAL"|"VERTICAL", gap: number, pt: number, pr: number, pb: number, pl: number, w: number): FrameNode {
...
function assistantBubble(text: string): FrameNode {
...
function userBubble(text: string): FrameNode {
...
function typingIndicator(): FrameNode {
  const c = autoFrame("Typing Indicator", "HORIZONTAL", 4, 10, 12, 10, 12);
  c.fills = [solid(C.assistBubble)];
  c.cornerRadius = 8;
  for (let i=0; i<3; i++) {
    const d = figma.createEllipse();
    d.name = "Dot"; d.resize(6,6); d.fills=[solid(C.typingDot)];
    c.appendChild(d);
  }
  return c;
}

function tableRow(day: string, time: string, hours: string, hoursColor: {r:number,g:number,b:number} = C.primary): FrameNode {
  const row = fixedAutoFrame("Row "+day, "HORIZONTAL", 0, 2, 0, 2, 0, 356);
  row.primaryAxisAlignItems = "SPACE_BETWEEN";
  row.counterAxisAlignItems = "CENTER";
  const d = txt(day, 14, C.txtBody); d.layoutGrow = 1;
  row.appendChild(d);
  row.appendChild(txt(time, 14, C.txtMuted));
  const h = txt(hours, 14, hoursColor, "Regular"); h.textAlignHorizontal = "RIGHT"; h.resize(36, h.height);
  row.appendChild(h);
  return row;
}

function tableCard(title: string, rows: {day:string,time:string,hours:string}[], sumHrs: string, sumDays: string): FrameNode {
  const card = fixedAutoFrame("Table: "+title, "VERTICAL", 0, 0, 0, 0, 0, 380);
  card.fills = [solid(C.white)];
  card.cornerRadius = 8;
  card.strokes = [solid(C.cardBdr)]; card.strokeWeight = 1;
  ...
  // Header border
  const hSep = rect(380, 1, C.tblHdrBdr); hSep.name = "hdr-sep";
  ...
  // Summary divider
  const sDv = rect(356, 1, C.cardBdr); sDv.name = "sum-div";
  ...
  sum.appendChild(txt("📅 " + sumDays, 13, C.txtMuted, "Medium"));
  card.appendChild(sum);

  return card;
}

function statusBubble(text: string): FrameNode {
  const b = autoFrame("Status Bubble", "VERTICAL", 0, 10, 12, 10, 12);
  b.fills = [solid(C.successBg)]; b.cornerRadius = 8;
  b.strokes = [solid(C.successBdr)]; b.strokeWeight = 1;
  b.appendChild(txt(text, 14, C.successTxt, "Medium", 340));
  return b;
}

function confirmCard(showBtns: boolean = true): FrameNode {
  const card = autoFrame("Confirm Card", "VERTICAL", 12, 12, 12, 12, 12);
  card.fills = [solid(C.white)]; card.cornerRadius = 8;
  card.strokes = [solid(C.cardBdr)]; card.strokeWeight = 1;
  ...
  if (showBtns) {
    ...
    sBtn.fills=[solid(C.primary)]; sBtn.cornerRadius=40;
    ...
    nBtn.fills=[solid(C.white)]; nBtn.cornerRadius=40;
    nBtn.strokes=[solid(C.cardBdr)]; nBtn.strokeWeight=1;
...
function panelHeader(title: string): FrameNode {
  const h = fixedAutoFrame("Header", "HORIZONTAL", 0, 0, 20, 0, 20, PANEL_W);
  h.resize(PANEL_W, 60); h.primaryAxisSizingMode = "FIXED";
  ...
  if (title === "Your Chats") {
    ...
    ic.fills=[solid(C.sparkleBg)]; ic.cornerRadius=12; ic.resize(24,24);
    ic.primaryAxisSizingMode="FIXED"; ic.counterAxisSizingMode="FIXED";
    ic.counterAxisAlignItems="CENTER"; ic.primaryAxisAlignItems="CENTER";
  ...
  // Bottom border line
  const bdr = rect(PANEL_W, 1, C.hdrBdr); bdr.name="hdr-border";
  bdr.layoutPositioning = "ABSOLUTE"; bdr.x=0; bdr.y=59;
  h.appendChild(bdr);
  return h;
}

function composer(): FrameNode {
  const outer = fixedAutoFrame("Composer", "VERTICAL", 0, 0, 16, 16, 16, PANEL_W);
  outer.fills = [solid(C.white)];

  // Top border
  const topB = rect(PANEL_W, 1, C.compTopBdr); topB.name="comp-top-border";
  topB.layoutPositioning="ABSOLUTE"; topB.x=0; topB.y=0;
  outer.appendChild(topB);

  const form = fixedAutoFrame("Form", "HORIZONTAL", 8, 8, 12, 8, 12, 388);
  form.cornerRadius=40; form.strokes=[solid(C.compBdr)]; form.strokeWeight=1;
  form.fills=[solid(C.white)];
  form.resize(388, 56); form.primaryAxisSizingMode="FIXED";
  form.counterAxisAlignItems="CENTER";

  form.appendChild(txt("📎", 16, C.txtMuted));
  const ph = txt("Ask AURA", 16, C.txtPlaceholder); ph.layoutGrow=1;
  form.appendChild(ph);

  const sendCirc = figma.createEllipse();
  sendCirc.name="Send"; sendCirc.resize(42,42);
...
function panel(title: string, body: FrameNode, showComposer: boolean = true): FrameNode {
  const p = fixedAutoFrame("Panel", "VERTICAL", 0, 0, 0, 0, 0, PANEL_W);
  p.fills=[solid(C.white)]; p.cornerRadius=12;
  p.strokes=[solid(C.cardBdr)]; p.strokeWeight=1;
  p.effects=[{
    type:"DROP_SHADOW", visible:true, blendMode:"NORMAL",
    color:{r:0.059,g:0.09,b:0.165,a:0.22},
    offset:{x:0,y:16}, radius:60, spread:0,
  }];
...
function chatBody(msgs: {role:string, node:FrameNode}[]): FrameNode {
...
function convFrame(name: string, msgs: {role:string, node:FrameNode}[]): FrameNode {
...
function historyRow(title: string, preview: string, date: string): FrameNode {
  const row = fixedAutoFrame("Chat: "+title, "HORIZONTAL", 12, 12, 16, 12, 16, 388);
  row.counterAxisAlignItems="MIN";

  const ic = autoFrame("Icon", "HORIZONTAL", 0, 8, 8, 8, 8);
  ic.fills=[solid(C.sparkleBg)]; ic.cornerRadius=18; ic.resize(36,36);
  ic.primaryAxisSizingMode="FIXED"; ic.counterAxisSizingMode="FIXED";
  ic.counterAxisAlignItems="CENTER"; ic.primaryAxisAlignItems="CENTER";
...
function historyDefault(): FrameNode {
  ...
  search.fills=[solid(C.white)]; search.cornerRadius=6;
  search.strokes=[solid(C.compBdr)]; search.strokeWeight=1;
  search.resize(388, 36); search.primaryAxisSizingMode="FIXED";
  search.counterAxisAlignItems="CENTER";
  const sIcon = txt("🔍", 14, C.txtMuted);
  sIcon.layoutPositioning="ABSOLUTE"; sIcon.x=10; sIcon.y=8;
  ...
  list.fills=[solid(C.white)]; list.cornerRadius=8;
  list.strokes=[solid(C.hdrBdr)]; list.strokeWeight=1;
  for (let i=0; i<items.length; i++) {
    list.appendChild(historyRow(items[i].title, items[i].preview, items[i].date));
    if (i < items.length-1) {
      const sep = rect(388, 1, C.rowSep); sep.name="sep"; sep.layoutAlign="STRETCH";
...
function historyEmpty(): FrameNode {
  ...
  const clr = txt("✕", 13, C.txtMuted); search.appendChild(clr);
  body.appendChild(search);

  const empty = fixedAutoFrame("EmptyState", "VERTICAL", 12, 32, 24, 32, 24, 388);
  empty.fills=[solid(C.white)]; empty.cornerRadius=8;
  empty.strokes=[solid(C.hdrBdr)]; empty.strokeWeight=1;
  empty.counterAxisAlignItems="CENTER";

  const eic = autoFrame("EIcon", "HORIZONTAL", 0, 10, 10, 10, 10);
  eic.fills=[solid(C.emptyCircle)]; eic.cornerRadius=20; eic.resize(40,40);
  eic.primaryAxisSizingMode="FIXED"; eic.counterAxisSizingMode="FIXED";
  eic.counterAxisAlignItems="CENTER"; eic.primaryAxisAlignItems="CENTER";
  eic.appendChild(txt("🔍", 15, C.txtSlate));
  empty.appendChild(eic);

  const et = txt("No chats found", 14, C.txtDark, "Bold");
  et.textAlignHorizontal="CENTER"; empty.appendChild(et);
  const ed = txt("Try searching by request type, employee name, or status.", 13, C.txtSlate, "Regular", 260);
  ed.textAlignHorizontal="CENTER"; empty.appendChild(ed);
...
function launcherBtn(): FrameNode {
  const l = autoFrame("AURA Launcher", "HORIZONTAL", 8, 12, 20, 12, 20);
  l.cornerRadius=24; l.counterAxisAlignItems="CENTER";
  l.fills=[{
    type:"GRADIENT_LINEAR",
    gradientTransform:[[1,0,0],[0,1,0]],
    gradientStops:[
      {position:0, color:{...C.gradStart, a:1}},
      {position:1, color:{...C.gradEnd, a:1}},
    ],
  }];
  l.effects=[
...
function launcherTooltip(): FrameNode {
  const t = fixedAutoFrame("Tooltip", "VERTICAL", 4, 12, 16, 12, 16, 286);
  t.fills=[solid(C.white)]; t.cornerRadius=8;
  t.strokes=[solid(C.cardBdr)]; t.strokeWeight=1;
...
function ann(label: string, value: string): FrameNode {
  const a = autoFrame("Ann", "HORIZONTAL", 4, 3, 8, 3, 8);
  a.fills=[solid(C.annBg)]; a.cornerRadius=4;
  a.strokes=[solid(C.annBdr)]; a.strokeWeight=1;
  a.appendChild(txt(label, 10, C.txtMuted, "Medium"));
  a.appendChild(txt(value, 10, C.txtBody, "Bold"));
  return a;
}

function annBlock(items: [string,string][]): FrameNode {
...
async function main() {
  ...
  f01.x=x; f01.y=0; page.appendChild(f01);
  x += PANEL_W + FRAME_GAP;
  ...
  f02.x=x; f02.y=0; page.appendChild(f02);
  x += PANEL_W + FRAME_GAP;
  ...
  f03.x=x; f03.y=0; page.appendChild(f03);
  x += PANEL_W + FRAME_GAP;
  ...
  f04.x=x; f04.y=0; page.appendChild(f04);
  x += PANEL_W + FRAME_GAP;
  ...
  f05.x=x; f05.y=0; page.appendChild(f05);
  x += PANEL_W + FRAME_GAP;
  ...
  f06.x=x; f06.y=0; page.appendChild(f06);
  x += PANEL_W + FRAME_GAP;
  ...
  f07.x=x; f07.y=0; page.appendChild(f07);
  x += PANEL_W + FRAME_GAP;
  ...
  secLabel3.x = x; secLabel3.y = -60;
  page.appendChild(secLabel3);

  const compData: {label:string, builder:()=>FrameNode, anns:[string,string][]}[] = [
    {label:"Assistant Bubble", builder:()=>assistantBubble("Hello Jennings, How are you ! What can I do for you ?"),
     anns:[["bg","#E6F0FB"],["text","#333333 · 14px"],["radius","8px"],["padding","8px 12px"]]},
    {label:"User Bubble", builder:()=>userBubble("Yes, apply"),
     anns:[["bg","#F4F5FA"],["text","#111827 · 14px"],["radius","8px"],["align","right (ml-auto)"]]},
    {label:"Typing Indicator", builder:typingIndicator,
     anns:[["bg","#E6F0FB"],["dots","#6f7a8d · 6px"],["animation","aura-typing-pulse 3s"],["gap","4px"]]},
    {label:"Table Card", builder:()=>tableCard("Current Availability (48h)", curRows, "48 hrs total", "5 days/week"),
     anns:[["bg","#FFFFFF"],["border","#d8dce6 · 1px"],["radius","8px"],["hours","#0a68db (primary)"],["summary","#5c5c5c · 13px Medium"]]},
    {label:"Status Bubble (Success)", builder:()=>statusBubble("Done — sent to your manager.\n\nYour request ID is 437862374."),
     anns:[["bg","#ecfdf3"],["border","#b8e4c8 · 1px"],["text","#166534 · 14px Medium"],["radius","8px"]]},
    {label:"Confirmation Card", builder:()=>confirmCard(true),
     anns:[["bg","#FFFFFF"],["border","#d8dce6 · 1px"],["Submit btn","#0a68db radius 40px"],["Not now btn","#FFF border #d8dce6"],["btn text","13px Medium"]]},
    {label:"History Row", builder:()=>historyRow("Availability Request","Request sent to manager.","Today"),
     anns:[["icon bg","#e9f5ff · 36px circle"],["icon","#0868db sparkle"],["title","#111827 · 14px Bold"],["preview","#64748B · 13px"],["date","#64748B · 12px Medium"]]},
    {label:"Search Empty State", builder:()=>{
       const e = autoFrame("EmptyComp","VERTICAL",12,32,24,32,24);
       e.fills=[solid(C.white)]; e.cornerRadius=8; e.strokes=[solid(C.hdrBdr)]; e.strokeWeight=1;
       e.counterAxisAlignItems="CENTER"; e.resize(380,180); e.counterAxisSizingMode="FIXED"; e.primaryAxisSizingMode="FIXED";
       ...
       eic.appendChild(txt("🔍",15,C.txtSlate));
       e.appendChild(eic);
       const et=txt("No chats found",14,C.txtDark,"Bold"); et.textAlignHorizontal="CENTER"; e.appendChild(et);
       const ed=txt("Try searching by request type, employee name, or status.",13,C.txtSlate,"Regular",260);
       ed.textAlignHorizontal="CENTER"; e.appendChild(ed);
       return e;
     }, anns:[["icon bg","#f1f5f9 · 40px"],["title","#111827 · 14px Bold"],["desc","#64748B · 13px"],["min height","180px"]]},
    {label:"AURA Launcher Button", builder:launcherBtn,
     anns:[["gradient","#33C7EA → #2A2DBB"],["radius","24px (pill)"],["shadow","0 12px 28px rgba(42,45,187,0.35)"],["text","#FFF · 15px Bold"],["height","48px"]]},
    {label:"Launcher Tooltip", builder:launcherTooltip,
     anns:[["bg","#FFFFFF"],["border","#d8dce6"],["radius","8px"],["AURA AI","#5b2ad9 · 13px Bold"],["message","#1f2937 · 15px Bold"],["width","286px"]]},
  ];

  let cy = 0;
  for (const comp of compData) {
    const lbl = txt(comp.label, 14, C.txtMuted, "Bold");
    lbl.x = x; lbl.y = cy; page.appendChild(lbl);
    cy += 24;

    const node = comp.builder();
    node.x = x; node.y = cy; page.appendChild(node);

    const anns = annBlock(comp.anns);
    anns.x = x + 420; anns.y = cy; page.appendChild(anns);

    // Get approximate height of component
  ...
  tokenTitle.x = 0; tokenTitle.y = tokenY; page.appendChild(tokenTitle);
  ...
  for (const tok of tokens) {
    ...
    t.x = 0; t.y = ty; page.appendChild(t);
    ty += 24;
  }
